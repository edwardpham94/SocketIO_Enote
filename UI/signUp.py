...
heading = Label(frame, text="Sign up", fg="#86d9cf", bg="white",
...
full_name = Entry(frame, width=25, fg='black', border=0, bg="white",
                  font=('Bahnschrift SemiLight SemiConde', 11))
full_name.place(x=0, y=60)
full_name.insert(0, 'Fullname')
full_name.bind('<FocusIn>', on_enter)
full_name.bind('<FocusOut>', on_leave)


Frame(frame, width=290, height=2, bg='black').place(x=0, y=90)
...
def sign_up():
    print("here")
    _fullname = full_name.get()
    _username = username.get()
    _password = password.get()
    cmd = "signup"
    sendCommand(client, cmd)
    sendList(client, [_fullname, _username, _password])
...
Button(frame,
       ...
       font=('Bahnschrift SemiLight SemiConde', 12), command=sign_up).place(x=2, y=220)
